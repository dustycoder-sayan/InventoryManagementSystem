...
public class IssueProductDAO implements DatabaseConstants {
    ...
    public ResultSet getAllIssueProductDetails() {
        try {
            PreparedStatement results = conn.prepareStatement("SELECT * FROM " + ISSUE_PRODUCT_TABLE);
            if (!results.executeQuery().next())
                throw new SQLException("No Products");
            return results.executeQuery();
        } catch (SQLException e) {
            System.out.println("Couldn't fetch Issue Product Details: " + e.getMessage());
            return null;
        }
    }

    public ResultSet getIssueProductByProduct(int pId) {
        try {
            PreparedStatement result = conn.prepareStatement("SELECT * FROM " + ISSUE_PRODUCT_TABLE + " WHERE " + ISSUE_P_ID + "=" + pId);
            result.setInt(1, pId);
            if (!result.executeQuery().next())
                throw new SQLException("No Such Product");
            return result.executeQuery();
        } catch (SQLException e) {
            System.out.println("Couldn't fetch Details: " + e.getMessage());
            return null;
        }
    }

    public ResultSet getIssueProductByCustomer(int cId) {
        try {
            PreparedStatement result = conn.prepareStatement("SELECT * FROM " + ISSUE_PRODUCT_TABLE + " WHERE " + ISSUE_C_ID + "=?");
            result.setInt(1, cId);
            if (!result.executeQuery().next())
    ...
    public ResultSet getIssueProductByUser(int uId) {
        try {
            PreparedStatement result = conn.prepareStatement("SELECT * FROM " + ISSUE_PRODUCT_TABLE + " WHERE " + ISSUE_U_ID + "=?");
            result.setInt(1, uId);
            if (!result.executeQuery().next())
